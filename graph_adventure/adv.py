...
#random choice helper function. returns false if deadend
def choose_random(graph,room,symbol='?'):
    random_list=[]
...
#helper for updating my graph
def update_graph(graph,new_exits,new_room_id,prev_room_id,prev_direction):
    def get_opposite(dir):
        dir_dict = {'n':'s','s':'n','e':'w','w':'e'}
        return dir_dict[dir]

    #update previous room to reflect newly discovered room
    graph[prev_room_id][prev_direction] = new_room_id

    #what if we receive a previously traversed room?
    #do not add new dict item just update with appropriate opposite direction
    if new_room_id in graph:
        graph[new_room_id].update({get_opposite(prev_direction):prev_room_id})
    else:
    #otherwise add new room to graph and update with previous room id
        graph[new_room_id] = {}
        for exit in new_exits:
            graph[new_room_id].update({exit:'?'})
        graph[new_room_id].update({get_opposite(prev_direction):prev_room_id})

    return graph

#helper for checking q marks in dict in order to keep looping
def check_for_question_marks(graph):
   
    for d in graph:
        print(d)
        for direction in graph[d]:
            if graph[d][direction] == '?':
                
                return True

#helper for checking q marks in dict in order to keep looping
def check_for_question_marks_BFS(bfs_array):
    directions = []
    for direction in bfs_array:
      
      #there was a typo directions array was not outside function scope
        if bfs_array[direction] == '?':
...
def bfs(graph, dead_end_room, desired_room_symbol = True):
        queue = Queue()
        queue.enqueue([dead_end_room])
        found = []
        while queue.size() > 0:
            path = queue.dequeue()
            print("deque", path)
            v = path[-1]
            #collect other room vertices from current room
            
     
            if v not in found:
                #function will return direction (n.s.e.w) to first unexplored room if exists otherwise False
                
                room_id_or_false = check_for_question_marks_BFS(graph[v])
                if room_id_or_false:
                    return path,room_id_or_false #need to return appropriate path transated into n.s.e.w//add direction to return 2 items
                found.append(v)
                #collect potential room values to traverse
                # for key,value in graph[v].items():
                #     if value == '?':

                current_room_values = [value for key,value in graph[v].items()]

                for roomID in current_room_values:
                    # if roomID != '?':
                    new_path = list(path)
                   
                    new_path.append(roomID)
...
#start my traversal mapping-------------------------------
my_graph = {}
# need condition to loop forward

#set up initial room information and custom graph
roomID = player.currentRoom.id
newRoomID = ""
prev_direction = ""
currentExits = player.currentRoom.getExits()
my_graph.update({roomID:{}})
for exit in currentExits:
    my_graph[roomID].update({exit:'?'})
print(my_graph)

#start walking---------------------------------------------
while check_for_question_marks(my_graph):
    #grab current room again
    currentRoomID = player.currentRoom.id
    
    #choose random direction that has a ? else return False
    if currentRoomID not in my_graph:
        currentExits = player.currentRoom.getExits()
        my_graph.update({currentRoomID:{}})
        for exit in currentExits:
            my_graph[currentRoomID].update({exit:'?'})
    
    depthfirst_random_travel = choose_random(my_graph,currentRoomID)
    print("CHOICE RESULT",depthfirst_random_travel)


    #if False initiate BFS to find closest room with a ? 
    if depthfirst_random_travel is False:
        #set current room to the BFS found room
        #or get copy of path directions to next room and do what?
        print("CURRENT ROOM",currentRoomID)
        print("INITIATE BREDTH FIRST SEARCH",depthfirst_random_travel,my_graph,currentRoomID)
        print("BFS RESULTS",bfs(my_graph,currentRoomID))
        next_steps_arr, coordinate_of_next_q = bfs(my_graph,currentRoomID)

        print("BFS RESULTS",next_steps_arr,coordinate_of_next_q)
        print("TRAVERSAL PATH BEFORE SEARCH",traversalPath,len(traversalPath))
        #can you never have two directions leading to the same vertice?
        #translate room Ids to n.s.e.w
        translated = []
        for i in range(0, len(next_steps_arr)-1):
            bfs_room_number = next_steps_arr[i]

            print(bfs_room_number)
            for value in my_graph[bfs_room_number]:
                print("VALUEEEEEEEE", my_graph[bfs_room_number][value],bfs_room_number)
                if my_graph[bfs_room_number][value] == next_steps_arr[i +1]:
        ...
        for bfs_new_path in translated:
            ...
            traversalPath.append(bfs_new_path)
        ...
        print("RANDOM CHOICE AVAILABLE",depthfirst_random_travel)
        print("CURRENT ROOM",currentRoomID)
        new_direction = player.travel(depthfirst_random_travel)
        ###add activity travel to log#####
        traversalPath.append(depthfirst_random_travel)

        new_room_id = player.currentRoom.id
        new_room = player.currentRoom
        new_room_exits = player.currentRoom.getExits()

        my_graph = update_graph(my_graph,new_room_exits,new_room_id,currentRoomID,depthfirst_random_travel)
        print("NEWLY UPDATED GRAPH",my_graph)
        # print("Question mark check",check_for_question_marks(my_graph))


    
    print("len graph",len(roomGraph))
